...
export default function ClientLeadDetailPage() {
  ...
  if (!lead) return <p className="text-[#8c8c8c]">Lead not found or not assigned to your organisation.</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-[#262626] bg-[#111] p-5 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {OUTCOME_STATUSES.map(({ value, label }) => (
            <Button key={value} size="sm" variant={lead.status === value ? "default" : "outline"} disabled={saving} onClick={() => updateStatus(value)}>
              {label}
            </Button>
          ))}
        </div>
      ...
      <Button asChild variant="outline">
        <Link href="/client/leads">Back to leads</Link>
      </Button>
    </div>
  );
}
