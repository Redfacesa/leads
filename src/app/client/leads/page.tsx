  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#8c8c8c]">Leads purchased or assigned to your organisation</p>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-[#262626]">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[#8c8c8c]">Loading...</td></tr>
            ) : leads.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[#8c8c8c]">No leads yet. Browse the marketplace or wait for allocation.</td></tr>
            ) : (
              leads.map((lead) => (
                <tr key={lead.id} className="border-t border-[#262626]">
                  <td className="px-4 py-3">
                    <p className="font-mono text-white">{lead.lead_reference}</p>
                    <p className="text-[#bdbdbd]">{lead.first_name} {lead.last_name}</p>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/client/leads/${lead.id}`} className="text-[#dc2626] hover:underline">Open</Link>
                  </td>
                </tr>
              ))
            )}
